...
setwd("C:/Users/flyku/Desktop/iris3/2020071042004")
jobid <- "2020071042004"                          # user job id
expr_file = "Yan_2013_expression.csv"             # expression file name
delim <- ","                                      # expression file delimiter. comma or tab:(',' '\t')
label_file<-'Yan_2013_label.csv'                  # cell label file name
delimiter <- ','                                  # cell label file delimiter. comma or tab:(',' '\t')
is_imputation <- 'No'                             # Open imputation?
n_pc <- "10"                                      # Number of PCs
n_variable_feature <- "5000"                      # Number of variable features
resolution_seurat <- 0.8                          # Clustering resolution in Seurat 
label_use_predict <- '2'                          # If user upload cell label, label_use_predict=2; otherwize label_use_predict=0;
remove_ribosome <- 'No'                           # Remove ribosome genes?
upload_type <- "CellGene"                         # Uploaded gene expression file tyle: 1.CellGene; 2.TenX.folder 3.TenX.h5
species_file <- "Human"                           # Which species: Human or Mouse
